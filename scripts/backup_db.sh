#!/usr/bin/env bash
# Backup consistente de la base SQLite con retención configurable.
#
...
OUT_DB="$BACKUP_DIR/invoicing_${STAMP}.db"
...
# Usar sqlite3 .backup si está disponible (snapshot consistente aunque haya WAL).
...
gzip -9 "$OUT_DB"
echo "Backup: ${OUT_DB}.gz"

# Rotación: borrar copias más antiguas que RETAIN_DAYS (si RETAIN_DAYS > 0)
if [ -n "$RETAIN_DAYS" ] && [ "$RETAIN_DAYS" -gt 0 ] 2>/dev/null; then
  find "$BACKUP_DIR" -maxdepth 1 -name "invoicing_*.db.gz" -type f -mtime +"$RETAIN_DAYS" -delete 2>/dev/null || true
fi
