...
| **Backup desde /admin/ops** | Que existan `scripts/backup_db.sh` y `scripts/backup_storage.sh`, y que el directorio `backup/` sea escribible. El backup de DB se genera como `backup/invoicing_YYYYMMDD_HHMMSS.db.gz`. El backup de storage como `backup/storage_YYYYMMDD_HHMMSS.tar.gz`. En `/status` se comprueba `storage_writable`. |
...
## Restore de storage (XML + credenciales cifradas)

Si restauras DB, normalmente también quieres restaurar `storage/` (sobre todo `storage/xml` y `storage/credentials` cifradas).
